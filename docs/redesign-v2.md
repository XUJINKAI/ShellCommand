> 2026-09-12 用户修订：仅 ZIP 免签名分发，界面直接安装，不要求命令行。安装约定以 [最新安装契约](contracts/installation.md) 为准，替代本文旧的签名分发要求。

> 实施更新：用户已要求继续完成全部编码。P0 的干净 Win11、签名和真实 Explorer 检查保留为发布验收项，不阻塞后续开发。仅支持 v2，不实现旧配置兼容。
